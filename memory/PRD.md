...
## 2026-07-12 — Virtual Professional Recruiting Landing Page (/vas rewrite) — DONE, TESTED 8/8 + full E2E (iter77)
**Source:** client-provided spec doc (HCOB_VP_Landing_Page_Instructions-1.docx) — 9-section page + application form spec + compliance guardrails. User choices: replace /vas entirely · keep brand blue (#0044FF) · full build (collection + admin UI + notifications + confirmation email) · OG tags in index.html.
**Public page:** `/vas` fully rewritten (`VAsLanding.jsx`) with verbatim spec copy — hero "One Network. Multiple Ways to Earn.", 3 streams (Commission Agent / Virtual Gig Work / Team Lead Path), 4-step How It Works, Earning Opportunities (dark, includes mandatory "No income level is guaranteed" disclaimer — COMPLIANCE: do not remove), 10-service grid, 4 Why-HCOB differentiators, 6 requirements, inline application form, 6 FAQs. All CTAs smooth-scroll to #apply. Alias routes added: /virtual-professionals, /join (plus existing /earn, /work-with-us). index.html OG/twitter tags now target VP recruiting message. Page title/meta set per spec via useEffect.
**Form (`VPApplicationForm.jsx`):** full name (2+ words), email, phone w/ country-code prompt, country dropdown (~43) + auto TZ detect (Intl), stream chips (required), skill chips (optional), portfolio URL (shown only when a real skill selected), hours/day, sales experience, why-join (500-char counter), heard-from, required consent checkbox ("not salaried employment"), honeypot `website` field, captures ?src= UTM. Success → inline confirmation panel with verbatim copy.
**Backend (`routes/vp_applications.py`):** POST /api/public/vp-applications (honeypot silent-accept, 5/IP/hour rate limit, full-name + email + consent validation) → `vp_applications` collection (status new) → admin bell notify → fire-and-forget ops email (email_admins) + applicant confirmation email (Resend, _send_user_email w/ synthetic user dict). GET /api/admin/vp-applications?status= (items + counts), PATCH /api/admin/vp-applications/{id} (status: new/contacted/onboarding/accepted/rejected, admin_note). Registered in server.py.
**Admin UI:** `/ops/va-program/applications` (`AdminVPApplications.jsx`, VA Program nav → "Applications", UserPlus icon): status filter chips w/ counts, row list, detail dialog (all fields + portfolio link), status select + internal note (persist verified).
**Testing:** iteration_77 — 8/8 backend pytest (`tests/test_vp_applications.py`) + full frontend E2E (all sections, aliases, portfolio conditional, char counter, submit→confirmation, admin queue persistence). Test data cleaned (vp_applications emptied post-test).
**REDEPLOY needed** — user pushes preview → production.


## 2026-07-12 — Fixed Pool Model v2.0 Commission Engine (HCOB_Commission_Structure_v2.pdf) — DONE, TESTED (iter78: 14/14 pool engine + pipeline regression + 12/12 legacy suite updated)
**Supersedes ALL prior commission schedules.** User choices: transition per doc §10 · separate job_profit field at Paid · Ops 10% paid to PM (Mechie) through pipeline · fully automatic team-lead qualification · bonuses (+$25 multi-trade/+$10 RUSH) SKIPPED this phase.
**Engine (`va_commission.py`):** pool = base × category_rate(tier); split 75% agent (kind `pool_agent`) / 15% team lead (kind `team_override`, level 1) / 10% ops (kind `ops_share` → user with is_program_manager=true). Sum never exceeds pool. Unclaimed lead share retained by Company (recorded as lead_share_retained + lead_share_reason on agent doc). Categories: A routine 10/12.5/15 · B premium cleans 12/15/18 · C trades 12/15/18 · D recurring tail 15/10/5 (visits 1-3/4-12/13+, no cap, 90-day inactivity ends tail → chain restarts at visit 1) · E commercial 5% monthly revenue · F virtual projects 12/15/18 · G virtual retainers 5% (digital + is_recurring). Base = job_profit (A-D,F) or job_value revenue (E,G). Tiers from cumulative paid leads: Agent <25, Senior 25+, Elite 60+ (never backward; legacy paid leads count as credit). Rates admin-editable (app_settings.pool_rates, doc defaults hardcoded); 75/15/10 split FIXED (not editable).
**Team system REPLACED:** L2 overrides REMOVED — one level deep only, 3-5 agents max per lead. Lead auto-qualification on every split (`_team_lead_status`): approved + is_team_lead + Senior tier + production ≥8 paid jobs/mo (paused when current+last 2 months all <8; grace window if team_lead_since within 2 months). Promote endpoint rejects sub-Senior VAs; promoting clears team membership; team cap 5 enforced.
**Transition rules honored:** legacy commissions (no engine field) past 'calculating' frozen at prior rates; frozen statuses never recomputed; recurring visit chains credit legacy 'routine' leads. DUPE FIX: `_find_duplicate_lead` now allows same-VA resubmission of their OWN completed/paid client (repeat visit for D tail); different-VA still blocked 409 within 90 days.
**New lead fields:** `is_recurring` (VA submit checkbox + LeadDetail edit), `job_profit` (required at Paid for A-D/F; job_value required for E/G — enforced 400 in stage endpoint). Commercial log-revenue now creates full pool split + blocks if VA inactive (§6).
**Endpoints changed:** /pm/commission-settings GET/PUT new shape {pool_rates, pool_split, tier_thresholds, category_labels, defaults}; per-VA commission-overrides endpoints DELETED (+ VACommissionOverrides.jsx deleted); /pm/teams returns tier + qualification + production; /va/team returns qualification + lead_share_pct; /va/dashboard adds agent_tier.
**Frontend:** AdminVARates rewritten (pool grid editor + split banner), AdminVATeams rewritten (qualification badges + production rows), VAMyTeam rewritten (qualification card, verified live), VASubmitLead recurring checkbox, Pipeline/board profit-vs-revenue inputs, LeadDetail job_profit field + pool-breakdown panel, VADashboard agent-tier-card.
...
## Worker Signup Questionnaire Redesign — FRD Addendum A (Jun 14, 2026) — ALL 3 PHASES COMPLETE
- **Taxonomy v2**: work classes (general_labor/specialist), 12 general skills (cleaning+labor groups), 9 specialist trades (painting, landscaping, carpet cleaning, pressure washing, carpentry, handyman, junk removal, plumbing†, electrical†; † = licensed), 3 work attributes (never shown as skills), cert tags (forklift/CDL as doc-only badges).
- **Worker wizard** (`WorkerQuestionnaire.jsx`) at /crew/onboarding (new signups auto-land there via PublicOnly) and /crew/questionnaire (from profile). Steps: classes → general skills (driving/delivery gated by vehicle) → trade claims (admin-defined equipment checklist + detail fields + photo proof + per-trade experience + license # for licensed) → certs (links to badge system) → attributes (+bilingual language) → availability/vehicle (CDL removed) → review/finish. Incomplete claims block profile completion (specialist_trade_incomplete).
- **Backend**: `routes/trades.py` (worker claim lifecycle incomplete→pending→verified/returned; admin trade CRUD, review queue, verify/return-with-note/extend-grace, metrics), `worker_taxonomy.py` (legacy `skills` array derived from v2 fields — dispatch/filters/blasts unchanged; 30-day grace for migrated claims; 6h grace-expiry resync loop; lossless migration w/ skills_legacy_backup + legacy_unmapped_skills + synonym map).
- **Migration**: 839 workers migrated idempotently (questionnaire_version=2). NOTE: first run wiped legacy "cleaning" chips → fixed with synonym map (cleaning→routine_cleaning) + repaired 279 test accounts. Production deploy will run the FIXED migration.
- **Admin**: /ops/trades (Review Queue w/ side-by-side checklist+photos, Trade Manager no-code checklist editor, Metrics), roster filters (work_class, trade+status, attributes), WorkerDetail trades card, gig target_trade select (blast + accept gated to active trade holders).
- **Badges**: seeded doc-only Forklift + CDL badges (skill_tag feeds dispatch skills on approval; no test required).
- Tested: iteration_79 — backend 15/15, frontend wizard/profile/admin flows pass; register→onboarding redirect + roster filters + gig dialog self-verified after fix.

## Gig Posting Templates & Specialist Interest Flow — FRD Addendum B, Phases 1 & 2 (Jun 15, 2026) — DONE, TESTED (iteration_80)
- **Phase 1 — Specialist posting template**: `template: labor_shift | specialist_project` on GigIn. Specialist gigs require ≥2 photos (first = card thumbnail, via POST /api/admin/gig-photos), condition notes, quantity+unit, materials split (we provide / you bring), est hours range. Pay modes: flat / hourly_estimate / range ($min–$max + mandatory reason). Date modes: fixed / window (pick your day at claim) / tbd (interest only). Validation in `routes/gigs.py::_specialist_fields` (400 "Specialist project incomplete" on missing pieces). Admin UI: `CreateGigDialog.jsx` + `SpecialistGigFields.jsx` — date/time pickers hidden unless fixed date mode.
- **Phase 2 — Interest-first mechanics** (`routes/gig_interest.py`): gigs with open variables (pay_mode=range OR date_mode=tbd → `takesInterestOnly` in `lib/specialist.js`) show "I'm Interested" instead of Claim. POST/DELETE /api/gigs/{id}/interest (note + availability, dedupe-updates, interest_count maintained, admins notified). POST /api/gigs/{id}/view — per-worker deduped view tracking → view_count. GET /api/admin/gigs/{id}/interests — enriched queue (worker name/phone/zip/badges/verified trades/completions/rating). Worker UI: SPECIALIST pill + scope line + INTERESTED pill on feed cards (`WorkerFeed.jsx`), InterestPanel with express/withdraw cycle on `WorkerGigDetail.jsx`. Admin UI: `GigInterestsCard.jsx` on /ops/assignments/{id} with views badge; PAY stat tile now shows payLine (range/hourly est) instead of $0.00 fallback.
- Fixed 2 JSX syntax errors left by previous session (duplicated Bit tail in WorkerFeed.jsx; unclosed needsFixedDate conditional in CreateGigDialog.jsx).
