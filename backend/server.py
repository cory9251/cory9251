...
async def on_startup():
    """Startup orchestration is delegated to `startup.py`. This keeps
    server.py thin and makes the 4 startup phases testable in isolation:
      1. Indices         — Mongo indices (idempotent)
      2. Migrations      — legacy backfills + slot reconciliation
      3. Seeds           — admin / PM / pitch templates / OWNER_RESET failsafe
      4. Storage init    — object storage (sync — must happen before tasks)
      5. Background tasks — message digest + reminders daemons
    """
    from startup import (
        ensure_indices,
        run_migrations,
        seed_accounts_and_templates,
